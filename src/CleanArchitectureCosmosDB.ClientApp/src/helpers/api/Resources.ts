...
export class ToDoItemClient {
     ...
     * Search
     */
    search(query: SearchToDoItemQuery): Promise<DataTablesResponse> {
    ...
    protected processSearch(response: Response): Promise<DataTablesResponse> {
        const status = response.status;
        let _headers: any = {}; if (response.headers && response.headers.forEach) { response.headers.forEach((v: any, k: any) => _headers[k] = v); };
        if (status === 200) {
            return response.text().then((_responseText) => {
        ...
        return Promise.resolve<DataTablesResponse>(<any>null);
    }
}
